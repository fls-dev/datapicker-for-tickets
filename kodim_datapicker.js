          ...
          let d = new Date(),firstDayOfMonth = new Date(y, m, 7).getDay()
            ...
            do {
                ...
                if (i === KodimLibSettings.currDay && chkM === KodimLibSettings.currMonth) {
                    html += `<div class="k-days today" data-mess="${mes}">${i}</div>`;
                } else if (dow === 6 &&  noWeek || dow === 0 && noWeek) {
                    html += `<div class="k-days c-weekend" data-mess="${mes}">${i}</div>`;
                } else if (new Date(y, m, i) < new Date() && KodimLibSettings.minDateToday === 1) {
                    html += `<div class="k-days c-past-days" data-mess="${mes}">${i}</div>`;
                }else {html += `<div class="k-days normal" data-mess="${mes}">${i}</div>`;}
                if (dow === 0) {
                    html += '</div>';
